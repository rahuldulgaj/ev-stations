...
<div class="col-md-12">
    <div class="card shadow mb-4">
        ...
        <form action="{{route('evstation.search')}}" method="GET" class="form-horizontal">
            <div class="card-body">
            ...
            <div class="border-top">
                <div class="card-body">
                    ...
                    <a href="{{route('evstation.index')}}" class="btn btn-md btn-danger">Clear</a>
                    <a class="btn btn-md btn-info " href="{{ route('evstation.create') }}" ><i class="fa fa-plus"> </i>Add EV Station</a>
                </div>
            </div>
        </form>
    </div>
            ...
            <div class="card-body">
              <div class="table-responsive">
                ...
                <div class="col-sm-12 col-md-6"><div id="dataTable_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="dataTable"></label></div></div></div><div class="row"><div class="col-sm-12"><table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                  ...
                  @foreach($evstations as $evstation)
                 
                                    <tr role="row" class="odd">
                                        <th>{{$loop->index+1}}</th>
                                        <td class="sorting_1">{{$evstation->name}}</td>
                                        <td class="sorting_1">{{$evstation->ct_code}}</td>
                                        <td class="sorting_1">{{$evstation->ct_company}}</td>
                                        <td class="sorting_1"><img src="{{ url('storage/uploads/gallery/evstation/'.$evstation->image) }}" alt="" title="" /></td>
                                        <td class="sorting_1">{{$evstation->created_at}}</td>
                                        @if($evstation->status== '1')
                                            <td>Active</td>
                                        @else
                                            <td>Deactive</td>
                                         @endif
